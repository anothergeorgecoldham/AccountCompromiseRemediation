...
<#
.SYNOPSIS
    Interactive account compromise remediation using app-only certificate auth.

.DESCRIPTION
    Identical UX to Invoke-AccountRemediation.ps1 (menu mode, -RunAll, -Actions,
    -DryRun, -SkipForensics, -SkipPreflight) but authenticates as a dedicated
    Entra app registration via certificate. The script runs with the app's
    Graph application permissions and Exchange Online role assignments — the
    operator does NOT need to hold privileged Entra roles.

    Configuration (TenantId, ClientId, CertificateThumbprint, Organization) is
    loaded from config/app-auth.json by default; any value can be overridden by
    a script parameter. Run Setup-ACRAppRegistration.ps1 once to provision the
    app, certificate, and consent.

.PARAMETER UserPrincipalName
    The UPN of the compromised user account. (Mandatory.)

.PARAMETER ConfigPath
    Path to the JSON config file. Defaults to ./config/app-auth.json relative to
    the repository root.

.PARAMETER TenantId
    Override for the tenant ID/domain.

.PARAMETER ClientId
    Override for the app registration's Application (client) ID.

.PARAMETER CertificateThumbprint
    Override for the certificate thumbprint (cert must be in CurrentUser\My or
    LocalMachine\My).

.PARAMETER Organization
    Override for the EXO organization (e.g. contoso.onmicrosoft.com).

.PARAMETER OperatorUpn
    Optional override for operator attribution in the audit log. Defaults to the
    current Windows identity.

.PARAMETER RunAll
    Run all applicable remediation actions without showing the menu.
...
.PARAMETER DryRun
    Show what would be done; make no changes.

.PARAMETER Actions
    Run only the named actions (e.g. "ResetPassword","RevokeTokens","EnforceMFA").

.PARAMETER OutputPath
    Base path for logs, forensics, and reports. Defaults to ./output.

.PARAMETER MaxRetries
    Maximum retry attempts for throttled Graph calls. Default 3.

.PARAMETER RetryDelaySeconds
    Base delay between retries. Default 10.

.EXAMPLE
    .\Invoke-AccountRemediationAppAuth.ps1 -UserPrincipalName user@contoso.com -RunAll

.EXAMPLE
    .\Invoke-AccountRemediationAppAuth.ps1 -UserPrincipalName user@contoso.com `
        -ConfigPath D:\secrets\acr-app-auth.json -DryRun

.EXAMPLE
    .\Invoke-AccountRemediationAppAuth.ps1 -UserPrincipalName user@contoso.com `
        -TenantId contoso.onmicrosoft.com -ClientId <guid> -CertificateThumbprint <thumb>
#>
...
# ── Resolve config (file → parameter override precedence) ────────────────────
function Resolve-ACRAppAuthConfig {
    param(
        [string]$ConfigPath,
        [string]$TenantId,
        [string]$ClientId,
    ...
    if (-not $ConfigPath) {
        $ConfigPath = Join-Path $PSScriptRoot '..\config\app-auth.json'
    }
    $ConfigPath = [System.IO.Path]::GetFullPath($ConfigPath)

    if (Test-Path $ConfigPath) {
        try {
            $raw = Get-Content $ConfigPath -Raw | ConvertFrom-Json -ErrorAction Stop
            if ($raw.tenantId)              { $config.TenantId              = $raw.tenantId }
            if ($raw.clientId)              { $config.ClientId              = $raw.clientId }
            if ($raw.certificateThumbprint) { $config.CertificateThumbprint = $raw.certificateThumbprint }
            if ($raw.organization)          { $config.Organization          = $raw.organization }
            Write-Verbose "Loaded app-auth config from $ConfigPath"
        } catch {
            Write-Warning "Failed to parse $ConfigPath as JSON: $($_.Exception.Message). Falling back to parameters/defaults."
        }
    } else {
        Write-Verbose "No config file at $ConfigPath. Using parameters only."
    }
    ...
    foreach ($k in @('TenantId','ClientId','CertificateThumbprint')) {
        if (-not $config[$k]) { $missing += $k }
    }
    if ($missing.Count -gt 0) {
        throw "Missing required app-auth setting(s): $($missing -join ', '). Provide via $ConfigPath, the corresponding -$($missing -join ', -') parameter, or run Setup-ACRAppRegistration.ps1."
    }
...
function Show-Banner {
    param([string]$DisplayName, [string]$UPN, [PSCustomObject]$UserContext, [PSCustomObject]$Operator)

    Write-Host ''
    Write-Host '╔══════════════════════════════════════════════════════════════╗' -ForegroundColor Cyan
    Write-Host '║   Account Compromise Remediation - Interactive (App-Auth)   ║' -ForegroundColor Cyan
    Write-Host '╚══════════════════════════════════════════════════════════════╝' -ForegroundColor Cyan
    Write-Host ''
    ...
    Write-Host "Operator    : $($Operator.DisplayName) [$($Operator.Source)]" -ForegroundColor Yellow
    Write-Host "Auth Mode   : AppOnly (ClientId=$($Operator.AppId))" -ForegroundColor Yellow
    Write-Host ''
}
...
# ── Action Map (identical to delegated script) ───────────────────────────────
...
try {
    # ── 1. Resolve Config ────────────────────────────────────────────────────
    Show-SectionHeader 'Configuration'
    $appCfg = Resolve-ACRAppAuthConfig -ConfigPath $ConfigPath `
        -TenantId $TenantId -ClientId $ClientId `
        -CertificateThumbprint $CertificateThumbprint -Organization $Organization
    Write-Host "  TenantId  : $($appCfg.TenantId)"  -ForegroundColor Gray
    Write-Host "  ClientId  : $($appCfg.ClientId)"  -ForegroundColor Gray
    Write-Host "  Cert thumb: $($appCfg.CertificateThumbprint)" -ForegroundColor Gray
    if ($appCfg.Organization) {
        Write-Host "  EXO org   : $($appCfg.Organization)" -ForegroundColor Gray
    }

    # ── 2. Authentication ────────────────────────────────────────────────────
    Show-SectionHeader 'Authentication'
    Connect-ACRAppAuth -TenantId $appCfg.TenantId -ClientId $appCfg.ClientId `
        -CertificateThumbprint $appCfg.CertificateThumbprint | Out-Null
    Write-Host '  [✓] Microsoft Graph: app-only certificate auth' -ForegroundColor Green

    $exoConnected = $false
    if ($appCfg.Organization) {
        $exoConnected = Connect-ACRExchangeOnlineApp -Organization $appCfg.Organization `
            -AppId $appCfg.ClientId -CertificateThumbprint $appCfg.CertificateThumbprint
        if ($exoConnected) {
            Write-Host '  [✓] Exchange Online: app-only certificate auth' -ForegroundColor Green
        } else {
            Write-Host '  [!] Exchange Online not connected — mailbox/transport actions will be skipped' -ForegroundColor Yellow
        }
    } else {
        Write-Host '  [!] No Organization configured — Exchange Online will not be connected' -ForegroundColor Yellow
    }

    $operator = Get-ACROperatorIdentity -OperatorUpn $OperatorUpn

    # ── 3. Preflight Validation ──────────────────────────────────────────────
    if (-not $SkipPreflight) {
        Show-SectionHeader 'Preflight Validation'
        $preflight = Invoke-ACRPreflight -UserPrincipalName $UserPrincipalName -SkipAuditCheck -AuthMode AppOnly
        Write-Host (Format-ACRPreflightReport -PreflightResult $preflight)

        if (-not $preflight.Ready) {
            Write-Host ''
            Write-Host '  [✗] Preflight validation FAILED. Blocking issues:' -ForegroundColor Red
            foreach ($b in $preflight.BlockingFailures) {
                Write-Host "      - $($b.Name): $($b.Message)" -ForegroundColor Red
            }
            Write-Host ''
            Write-Host '  Resolve issues or re-run with -SkipPreflight to bypass (not recommended).' -ForegroundColor Yellow
            return
        }
    ...
    Show-Banner -DisplayName $userContext.DisplayName -UPN $UPN -UserContext $userContext -Operator $operator

    # ── 5. Start Logging ─────────────────────────────────────────────────────
    $logSession = Start-ACRLog -OutputPath $OutputPath -UserPrincipalName $UPN `
        -AlertSource 'Manual-AppAuth' -OperatorIdentity $operator -AuthMode AppOnly
    Write-Host "  [✓] Log session started — $($logSession.SessionId)" -ForegroundColor Green
    Write-Host "      Operator attribution: $($operator.DisplayName) ($($operator.Source))" -ForegroundColor Gray
    ...
    # ── 7. Build applicable action list ──────────────────────────────────────
...
Operator  : $($operator.DisplayName) [$($operator.Source)]
App (SP)  : $($operator.AppId)
